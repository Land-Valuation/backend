...
const ModelBase = () => {
  ...
  const determineModelBaseMode = (roles) => {
    if (roles && roles.length > 0) {
      if (
        roles.some((role) =>
          [USER_ROLES.ROLE_LOCAL_MANAGER, USER_ROLES.ROLE_LOCAL_USER].includes(
            role
          )
        )
      ) {
      ...
      if (
        roles.some((role) =>
          [
            USER_ROLES.ROLE_CENTRAL_MANAGER,
            USER_ROLES.ROLE_CENTRAL_USER,
          ].includes(role)
        )
      ) {
  ...
  return (
    <>
      ...
      >
        ...
        {viewMode === MODEL_BASE_MODE.UNAUTHORIZED && (
          <Box
            sx={{
              display: "flex",
